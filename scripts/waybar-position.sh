...
RICE_ISO_CONFIG_FILE="$HOME/rice-iso/source/dots/waybar/config.jsonc"
...
for base in (os.path.expanduser('~/dots/hypr/modules'), os.path.expanduser('~/.config/hypr/modules'), os.path.expanduser('~/rice-iso/source/dots/hypr/modules')):
    ...
    if os.path.exists(deco):
        ...
        if 'leaf = "specialWorkspaceOut"' in content:
...
def update_waybar_config(filepath, pos):
    ...
    if pos in ('left', 'right'):
        content = re.sub(r'\"height\":\s*\d+', f'\"height\": {$mon_h}', content)
...
for p in ('$CONFIG_FILE', '$DOTS_CONFIG_FILE', '$RICE_ISO_CONFIG_FILE'):
    update_waybar_config(p, '$target_pos')
" 2>/dev/null || true
...
cmd_menu() {
    ...
    cur=$(get_current_pos)
    local selected
    selected=$(printf "󰋜  Top (Horizontal Fullscreen)\n󰋚  Right (Vertical Fullscreen)\n󰋙  Bottom (Horizontal Fullscreen)\n󰋛  Left (Vertical Fullscreen)" | rofi -dmenu -i -p "Waybar Position:")
    [[ -z "$selected" ]] && exit 0
    case "$selected" in
