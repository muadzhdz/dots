...
# ─────────────────────────────────────────────────────────────────────────────
# Pure Monochrome Theme Controller (Dark Only) - No Matugen, No Blur
# ─────────────────────────────────────────────────────────────────────────────
...
apply_dark() {
    ...
    hyprctl eval 'hl.config({ decoration = { active_opacity = 1.0, inactive_opacity = 1.0, blur = { enabled = false } }, general = { col = { active_border = { colors = {"rgba(ffffffff)"} }, inactive_border = "rgba(333333ff)" } } })' >/dev/null 2>&1 || true

    mkdir -p "$HOME/.config/mako" "$HOME/.config/rofi" "$HOME/.config/swayosd" "$HOME/.config/waybar" "$HOME/.config/kitty" "$HOME/.config/ghostty" "$HOME/.config/gtk-3.0" "$HOME/.config/gtk-4.0" "$HOME/.config/btop/themes"
    ...
    cp "$THEMES_DIR/dark/ghostty.conf" "$HOME/.config/ghostty/theme.conf"
...
case "${1:-dark}" in
    ...
    *)
        echo "Usage: theme-mode.sh {dark|get}"
        exit 1
        ;;
esac
